...
async def transform(
    ...
    try:
        ...
        if exc.code == 503:
            return {"error": "Google's AI model is temporarily overloaded — this usually clears up within a minute or two. Try again shortly."}
        return {"error": f"The AI request failed: {exc}"}
...
async def speak(text: str = Form(...), voice_speed: float = Form(1.0)):
    ...
    try:
        ...
        if exc.code == 503:
            return JSONResponse(
                {"error": "Google's AI model is temporarily overloaded — try again shortly."},
                status_code=503,
            )
        return JSONResponse({"error": f"TTS generation failed: {exc}"}, status_code=502)
